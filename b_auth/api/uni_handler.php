...
switch($_GET['type'] ?? '?') {
    ...
    case 'log':
        ...
        die($wrapped_log);
case 'hwid':
    $hwid_out = api\admin\reset_user_hwid($c_con, $_POST['program_key'], $_POST['username']);

    $wrapped_log = responses::wrapper($hwid_out);

    die($wrapped_log);
    case 'reset_psswd':
        $hwid_out = api\main\send_reset_email($c_con, $_POST['username']);
