...
function fetch_user_data(mysqli_wrapper $c_con, $username){
    $u_q = $c_con->query('SELECT * FROM c_program_users WHERE c_username=?', [$username]);
...
function send_reset_email_user(mysqli_wrapper $c_con, $username){
    $user_data = fetch_data($c_con, $username);
...
function reset_password_with_code_user(mysqli_wrapper $c_con, $code, $new_password){
    ...
    $c_con->query('UPDATE c_program_users SET c_password=? WHERE c_email=?', [$hashed_password, $code_data['c_email']]);
